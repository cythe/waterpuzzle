...
/* 当前管管们的状态 */
void printresults(struct _tube tubes[], int tube_cnt, int from, int to, int cnt)
{
    int split = tube_cnt/2;
    int blink;
	...
	{
	    if ((k==from || k==to) && j < cnt)
		blink=1;
	    else
		blink=0;
	    printcolor(tubes[k].colors[TUBE_FLOOR-j-1], blink);
	}
	printf("\n");
    }
...
void printtube(struct _tube *tube)
{
    ...
    for (int i = 0; i < TUBE_FLOOR; i++)
    {
	printcolor(tube->colors[TUBE_FLOOR-i-1], 0); 
	printf("\n");
    }
    printf(" ^^^ \n");
}
...
int is_complete(struct _tube tubes[], int tube_cnt)
{
    int i;
    for (i = 0; i < tube_cnt; i++)
    {
	if (is_sorted_full(&tubes[i]))
	    continue;
	else
	    return 0;
    }
...
int find_empty(struct _tube tubes[], int tube_cnt)
{
    int i;
    for (i = 0; i < tube_cnt; i++)
    {
	if (tubes[i].status == TUBE_EMPTY)
	    return i;
    }
    return -1;
}
